...
/* ── เพลงคลอ ────────────────────────────────────────────────────────────────
   อยู่ใน IIFE ของตัวเอง เพราะบล็อกข้างบนออกทันทีเมื่อไม่มีตารางรูป
   ปุ่มเพลงไม่ได้ผูกกับตารางรูปเลย จะพังตามกันไปด้วยไม่ได้

   ไม่มีการเล่นเองโดยไม่ได้กด — เบราว์เซอร์บล็อกเสียงที่เล่นเองอยู่แล้ว และแขก
   พันคนคูณไฟล์หลายเมกะไบต์คือแบนด์วิดท์ขาออกของบ้านที่ต้องแบ่งกับการอัพรูป */
(function () {
  const button = document.getElementById('music-toggle');
  const player = document.getElementById('music-player');
  const label = document.getElementById('music-label');
  if (!button || !player || !label) return;

  const KEY = 'wedding:music';
  player.volume = 0.35;

  // ป้ายบนปุ่มตามสถานะจริงของเครื่องเล่น ไม่ใช่ตามที่เราเพิ่งสั่งไป — เบราว์เซอร์
  // ปฏิเสธ play() ได้ตลอด (นโยบายเสียง เน็ตหลุด ไฟล์หาย) แล้วป้ายจะโกหกทันที
  function sync() {
    const playing = !player.paused;
    label.textContent = playing ? button.dataset.pause : button.dataset.play;
  ...
  player.addEventListener('error', () => {
    // เพลงเปิดไม่ได้ไม่ใช่เรื่องที่ต้องรบกวนแขกกลางงาน — เก็บปุ่มไปเงียบ ๆ
    button.hidden = true;
  });
  ...
  button.addEventListener('click', () => {
    if (player.paused) {
      player.play().then(() => {
        try { window.localStorage.setItem(KEY, 'on'); } catch { /* โหมดส่วนตัว */ }
      }).catch(() => sync());
    } else {
      player.pause();
      try { window.localStorage.removeItem(KEY); } catch { /* โหมดส่วนตัว */ }
    }
  });

  // เคยเปิดไว้ก็ลองเล่นต่อให้ ถ้าเบราว์เซอร์ไม่ยอมก็แค่ค้างที่ปุ่ม "เล่น" ตามเดิม
  let wanted = null;
  try { wanted = window.localStorage.getItem(KEY); } catch { /* โหมดส่วนตัว */ }
  if (wanted === 'on') player.play().catch(() => {});
