...
ตั้งสองรายการใน **Control Panel → Task Scheduler** (รันเป็น `root`, คำสั่งคือ
`/volume1/docker/wedding-share/scripts/ensure-up.sh`):

| ชนิด | ตั้งเมื่อไร | กันอะไร |
|---|---|---|
| **Triggered Task → Boot-up** | ครั้งเดียว | ไฟดับ / DSM อัปเดตแล้วรีบูตกลางดึก |
| **Scheduled Task → ทุก 5 นาที** | เปิดเฉพาะ 27–30 ส.ค. | อะไรก็ตามที่ทำให้เว็บล่มระหว่างงาน |

⚠️ **กับดัก `Last run time`** — ในหน้าตั้งเวลาแบบ "ทำซ้ำทุก N นาที" ช่อง
**Last run time ต้องเป็น `23:59`** ถ้าปล่อยเป็น `00:59` (ค่าที่ DSM ชอบเติมให้เอง)
งานจะเงียบหลังชั่วโมงแรกของทุกวัน — เคยทำให้ DDNS หยุดทำงานมาแล้วจริงในโปรเจกต์นี้

ซ้อมจริงก่อนงานหนึ่งครั้ง — สั่ง `sudo docker compose down` แล้วรอดูว่าเว็บกลับมาเอง
ภายใน 5 นาที **นี่คือตัวพิสูจน์ว่าตาข่ายใช้ได้ ไม่ใช่การที่ไฟล์สคริปต์มีอยู่**
