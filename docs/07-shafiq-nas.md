...
**โค้ดรองรับการสลับ encoder ผ่าน `.env` มาตั้งแต่แรกแล้ว แต่ที่ขาดคือ `docker-compose.yml`
ยังไม่เคย "ขอสิทธิ์" GPU จาก Docker daemon ให้คอนเทนเนอร์เลย — ต้องแก้สองที่ ไม่ใช่แค่ `.env`**

**1) `docker-compose.yml`** เพิ่ม GPU device reservation ให้บริการ `wedding-share`
(อยู่ในไฟล์แล้วตั้งแต่รอบที่ต่อสาย GPU):
...
⚠️ **นี่คือคำขอแบบ "บังคับ" ไม่ใช่ "มีก็ดีไม่มีก็ได้"** — ถ้า Docker daemon หา GPU
ไม่เจอตอนสร้างคอนเทนเนอร์ (ไดรเวอร์พัง, รีบูต DSM แล้วโมดูลไม่โหลด, การ์ดหลุด)
**`wedding-share` จะสร้างไม่ขึ้นเลย** เว็บทั้งเว็บล่มไปด้วย ต่างจากตอนที่ยังไม่ได้ขอ GPU
ที่พังแค่ตัวแปลงวิดีโอ ส่วนเว็บยังใช้ CPU รันต่อได้ปกติ — ดูหัวข้อ "GPU หายไปกะทันหัน"
ด้านล่างถ้าเจอเหตุนี้
...
### ถ้า GPU หายไปกะทันหัน — ถอยกลับ CPU ใน 1 นาที

Error ที่จะเห็นถ้า Docker หา GPU ไม่เจอตอนสร้างคอนเทนเนอร์ มักพูดถึง `nvidia` หรือ
`could not select device driver` ตัวที่ทำให้คอนเทนเนอร์สร้างไม่ขึ้นคือ **บล็อก
`deploy:` ใน `docker-compose.yml`** ไม่ใช่ค่าใน `.env` — ต้องแก้ทั้งสองที่ ไม่ใช่แก้ที่เดียว:

1. ลบบล็อก `deploy: resources: reservations: devices: ...` ทั้งก้อนออกจาก
   `docker-compose.yml` ชั่วคราว (คอมเมนต์ทิ้งไว้ก็ได้)
2. ใน `.env` เปลี่ยนกลับ `VIDEO_ENCODER=libx264` และคอมเมนต์/ลบสามบรรทัด
   `VIDEO_ENCODER_ARGS`/`FILM_ENCODER_ARGS`/`VIDEO_DECODER_ARGS` แบบ nvenc ออก
   (หรือปล่อยว่างไว้ให้ตกกลับค่าเริ่มต้นของ `.env.example`)
3. `sudo ./scripts/update.sh`

เว็บกลับมาใช้ CPU ได้ปกติทันที — ไม่มีอะไรต้องกู้คืนข้อมูล เพราะขั้นตอนนี้แตะแค่
การตั้งค่า ไม่แตะไฟล์รูป/วิดีโอ/ฐานข้อมูลเลย
