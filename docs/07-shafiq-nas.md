...
## เรื่อง GPU (GTX 1050 Ti) — ใช้งานจริงแล้ว

**สรุปสั้น: การ์ดใบนี้ใช้งานได้จริงแล้ว ยืนยันถึงระดับคอนเทนเนอร์ (ไม่ใช่แค่บนตัว DSM)
ต่อสายให้ทั้งคิวแปลงวิดีโอเบื้องหลังและการ export หนังใช้ GPU แล้ว**
...
### อุปสรรคเดิม — แก้ครบทั้งสี่ข้อแล้ว

ปัญหาไม่ได้อยู่ที่การ์ด อยู่ที่ **DSM/Xpenology** — ทั้งสี่ข้อนี้เคยเป็นตัวบล็อก
ตอนนี้แก้ครบแล้วทั้งหมด:

1. ~~Synology ไม่ได้แถม `nvidia.ko` มาให้~~ — ลงไดรเวอร์เรียบร้อย
2. ~~Container Manager ไม่มี `nvidia-container-toolkit`~~ — ติดตั้งแล้ว
   Docker daemon รู้จัก device driver ชื่อ `nvidia` แล้ว
3. ~~ffmpeg ในอิมเมจต้อง build มาพร้อม `--enable-nvenc`~~ — ffmpeg มาตรฐานของ Debian
   bookworm (ที่ `Dockerfile` ติดตั้งด้วย `apt-get install ffmpeg`) มี nvenc มาให้
   อยู่แล้วจริง ๆ (Debian ผูก nv-codec-headers ซึ่งเป็นแค่ header ไม่ใช่ driver
   ของ NVIDIA เอง จึงไม่ติดปัญหาลิขสิทธิ์ตอน build) — ไม่ต้อง build ffmpeg เองเลย
4. ความเสี่ยงเรื่องเคอร์เนลโมดูลผิดรุ่นทำให้ NAS บูตไม่ขึ้น — ผ่านมาได้แล้ว

### ตรวจให้แน่ใจอีกครั้งก่อนเปิดใช้จริง

```bash
lspci | grep -i nvidia          # DSM เห็นการ์ดไหม
ls /dev/nvidia*                 # ไดรเวอร์ขึ้นบนโฮสต์ไหม
docker run --rm --gpus all nvidia/cuda:12.4.1-base-ubuntu22.04 nvidia-smi   # Docker ส่ง GPU เข้าคอนเทนเนอร์ได้ไหม
docker exec wedding-share ffmpeg -encoders 2>/dev/null | grep nvenc         # ffmpeg ในอิมเมจของเรารู้จักไหม
```

ข้อสุดท้ายสำคัญที่สุด — ต้องรันหลัง deploy โค้ดที่ต่อสาย GPU แล้วเท่านั้น (ดูหัวข้อถัดไป)
ก่อนหน้านั้น `docker-compose.yml` ยังไม่ได้ขอ GPU ให้คอนเทนเนอร์เลย ต่อให้ไดรเวอร์กับ
toolkit พร้อมหมดแล้วก็ยังไม่เห็น nvenc ในนี้

### เปิดใช้งานจริง

**โค้ดรองรับการสลับ encoder ผ่าน `.env` มาตั้งแต่แรกแล้ว ส่วนการ "ขอสิทธิ์" GPU
จาก Docker daemon อยู่ในไฟล์ `docker-compose.gpu.yml` แยกต่างหาก**

**1) GPU อยู่ในไฟล์เสริม เปิดให้เองเมื่อใช้ได้จริง — ไม่ต้องแก้อะไร**

`deploy.resources.reservations.devices` ของ Compose เป็นคำขอแบบ **บังคับ**
ถ้า Docker หา GPU ไม่เจอตอนสร้างหรือสตาร์ทคอนเทนเนอร์ (รีบูตแล้วโมดูลไม่โหลด,
ไดรเวอร์พัง, การ์ดหลุด) **คอนเทนเนอร์จะไม่ขึ้นเลย = เว็บล่มทั้งเว็บ** และสิ่งที่ GPU
ไปช่วยคือคิวแปลง HEVC เบื้องหลังที่ไม่มีแขกคนไหนรออยู่ — เอาความเสี่ยงล่มทั้งเว็บ
ไปแลกกับงานที่ไม่มีใครรอคือแลกผิดฝั่ง

บล็อกนั้นจึงอยู่ใน **`docker-compose.gpu.yml`** ซึ่ง Compose จะโหลดก็ต่อเมื่อสั่ง `-f`
เท่านั้น ผลคือ:

| สั่งแบบนี้ | ได้อะไร |
|---|---|
| `sudo docker compose up -d` (ที่เอกสารบอกไว้หลายที่) | **CPU — ขึ้นได้เสมอ** |
| `sudo ./scripts/update.sh` | ตรวจ GPU ก่อน ใช้ได้ก็เปิดให้ ใช้ไม่ได้ก็ CPU |
| `sudo ./scripts/ensure-up.sh` | เหมือนกัน แต่ถ้ายกด้วย GPU แล้วไม่ขึ้น จะถอยเป็น CPU ให้เอง |

ตัวตรวจไม่ได้เดาจาก `/dev/nvidia*` แต่ **ลองสร้างคอนเทนเนอร์ที่ขอ GPU จริง ๆ**
(`docker run --rm --gpus all wedding-share:latest true`) เพราะไดรเวอร์ขึ้นบนโฮสต์
ไม่ได้แปลว่า `nvidia-container-toolkit` ยังต่ออยู่กับ Docker daemon

⚠️ **ห้ามเปลี่ยนชื่อไฟล์เป็น `docker-compose.override.yml`** — Compose โหลดชื่อนั้น
อัตโนมัติ ซึ่งจะทำให้ `docker compose up -d` เปล่า ๆ กลับไปเป็นคำขอบังคับอีกครั้ง

**2) ค่าตัวเข้ารหัสอยู่ในไฟล์เสริมแล้ว — ห้ามตั้งใน `.env`**

`docker-compose.gpu.yml` ตั้ง `VIDEO_ENCODER=h264_nvenc` (พร้อมอาร์กิวเมนต์ที่คู่กัน)
ให้เองอยู่แล้ว **จึงต้องไม่ตั้งค่าพวกนี้ใน `.env` ซ้ำอีก**

⚠️ เหตุผล — เคสนี้เกิดขึ้นจริงแล้ว: `.env` เก็บค่าไว้ถาวร แต่ GPU ต่อเข้าคอนเทนเนอร์
หรือเปล่าเปลี่ยนได้ทุกครั้งที่ยกคอนเทนเนอร์ พอ `.env` สั่ง nvenc ไว้แต่คอนเทนเนอร์
ขึ้นมาโดยไม่มี GPU ffmpeg จะล้มทุกครั้งที่ต้องเข้ารหัสด้วยข้อความ
...
**เว็บยังปกติดีทุกอย่าง** แขกอัพรูปได้ ดูรูปได้ แต่คลิปที่แขกส่งมาแปลงไม่ผ่านสักไฟล์
และหนังก็สร้างไม่ได้ — ไม่มีอะไรบอกจนกว่าจะมีคนเปิดวิดีโอบน Android แล้วเล่นไม่ได้

พอย้ายมาไว้ในไฟล์เสริม "ขอ GPU" กับ "สั่งใช้ nvenc" จะมาด้วยกันหรือไม่มาด้วยกันเสมอ
ขัดกันไม่ได้อีก · ถ้าเคยตั้งไว้ใน `.env` แล้ว ให้ล้างออกด้วย

```bash
sudo ./scripts/set-config.sh 'VIDEO_ENCODER=libx264' \
  'VIDEO_ENCODER_ARGS=-preset veryfast -crf 24 -profile:v high' \
  'FILM_ENCODER_ARGS=' 'VIDEO_DECODER_ARGS='
```

**ชั้นกันสุดท้ายในตัวแอป** — `activeEncoder()` ใน `src/lib/media.js` ทดลอง
**เข้ารหัสจริงหนึ่งเฟรม** ครั้งแรกที่ใช้ ถ้าไม่ผ่านจะถอยไป `libx264` ให้เองพร้อมเขียน
เหตุผลลง log ของคอนเทนเนอร์ ต่อให้ตั้งค่าขัดกันด้วยมือ คลิปของแขกก็ยังแปลงผ่าน

<details>
<summary>ค่าที่ไฟล์เสริมตั้งให้ (ไม่ต้องพิมพ์เอง)</summary>
...
⚠️ **อาร์กิวเมนต์ต้องเปลี่ยนคู่กับตัวเข้ารหัสเสมอ** — `libx264` ใช้ `-crf`
ส่วน `nvenc` ไม่รู้จักคำนี้ ต้องใช้ `-cq` แทน นี่คือเหตุผลที่ทั้งสี่บรรทัดถูกมัดไว้
ด้วยกันในไฟล์เดียว แทนที่จะให้ไปตั้งเองทีละค่า
...
**3) `scripts/export-film.sh`** ต่อสาย `--gpus all` ให้แล้วในไฟล์ ไม่ต้องแก้อะไรเพิ่ม —
คอนเทนเนอร์ชั่วคราวตัวนี้แยกจาก `wedding-share` อยู่แล้ว การขอ GPU ตรงนี้จึงไม่มีความเสี่ยง
กับเว็บที่ใช้งานจริงเลย (ถ้า GPU ไม่พร้อมตอนรัน สคริปต์นี้แค่ล้มเอง เว็บไม่กระทบ)

แล้ว `sudo ./scripts/update.sh` และเช็คทันทีว่าคอนเทนเนอร์ขึ้นจริง:

```bash
sudo docker compose ps                                                # ต้อง running (healthy)
docker exec wedding-share ffmpeg -encoders 2>/dev/null | grep nvenc   # ต้องเจอ
docker exec wedding-share env | grep VIDEO_ENCODER                    # ต้องเป็น h264_nvenc
```

**ยืนยันผลด้วยตัวเลข ไม่ใช่ด้วยการที่ไม่มี error** — อัพวิดีโอ HEVC จริงแล้วเทียบ fps:

```bash
docker compose logs -f wedding-share | grep -i fps
```
...
### ถ้า GPU หายไปกะทันหัน — ระบบถอยเองแล้ว

ไม่ต้องทำอะไร ตราบใดที่ยกคอนเทนเนอร์ด้วย `update.sh` หรือ `ensure-up.sh`
ทั้งสองตัวตรวจ GPU ก่อนเสมอ และ `ensure-up.sh` ยังถอยเป็น CPU ให้อีกชั้นถ้ายก
ด้วย GPU แล้วไม่ขึ้น เว็บจะกลับมาโดยเสียแค่ความเร็วของคิวแปลงวิดีโอเบื้องหลัง

ถ้าอยากบังคับ CPU ด้วยมือ: `sudo docker compose -f docker-compose.yml up -d`
(ไม่ใส่ `-f docker-compose.gpu.yml`) — ค่าใน `.env` ที่เป็น `h264_nvenc` ทิ้งไว้ได้
เพราะไม่มีคอนเทนเนอร์ไหนไปเรียก nvenc ถ้าไม่มี GPU ต่ออยู่ · อยากให้ชัวร์กว่านั้น
ก็ `sudo ./scripts/set-config.sh 'VIDEO_ENCODER=libx264' 'VIDEO_ENCODER_ARGS=-preset veryfast -crf 24 -profile:v high'`

### ตาข่ายรับ: `ensure-up.sh` กับ Task Scheduler

`ensure-up.sh` อ่านสถานะก่อนเสมอ — เว็บตอบอยู่ก็ **ออกทันทีโดยไม่แตะอะไรเลย**
(สำคัญ เพราะมันรันทุก 5 นาที ถ้าไป `up -d` ทับคอนเทนเนอร์ที่ทำงานดีอยู่ทุกรอบ
เว็บจะสะดุดเป็นระยะตลอดงานเพราะตัวที่ควรจะมาช่วย) · ลงมือเฉพาะตอนเว็บไม่ตอบ
และจดบรรทัดสรุปลง `/volume1/wedding/ensure-up.log` เฉพาะตอนที่ต้องซ่อมจริง

**ตรวจก่อนตั้งงาน — สคริปต์ต้องมีอยู่จริงบนเครื่องแล้ว**

```bash
ls -l /volume1/docker/wedding-share/scripts/ensure-up.sh
```

ไฟล์นี้มากับการอัปเดตโค้ดรอบที่เพิ่มตาข่ายรับ ถ้ายังไม่เคยรัน
`sudo ./scripts/update.sh` หลังจากนั้น ไฟล์จะยังไม่อยู่ แล้ว **งานที่ตั้งไว้จะล้ม
ทุกครั้งโดยไม่มีอะไรบอก** — ตารางงานจะดูเรียบร้อยดีทุกอย่าง

ต้องตั้ง **สองงาน คนละชนิดกัน** ใน `Control Panel → Task Scheduler`
เมนู `Create` มีสองทางแยก และนี่คือจุดที่พลาดกันบ่อยที่สุด

---

#### งานที่ 1 — กันรีบูต (Triggered Task)

`Create → **Triggered Task** → User-defined script`

| ช่อง | ค่า |
|---|---|
| Task | `wedding-ensure-up-boot` |
| User | `root` |
| **Event** | **`Boot-up`** |
| Task Settings → User-defined script | `/volume1/docker/wedding-share/scripts/ensure-up.sh` |

> 🔎 **ตัวจับผิดทันที**: งานชนิดนี้ **ไม่มีแท็บ `Schedule` เลย**
> ถ้าเห็นแท็บ Schedule แปลว่ากดมาจาก `Scheduled Task` ซึ่งผิดชนิด —
> กด Cancel แล้วเริ่มใหม่จาก `Create → Triggered Task`

กันอะไร: ไฟดับ · DSM อัปเดตแล้วรีบูตกลางดึก · ใครเผลอสั่ง reboot

---

#### งานที่ 2 — เฝ้าระหว่างงาน (Scheduled Task)

`Create → **Scheduled Task** → User-defined script`

แท็บ **General**: Task `wedding-ensure-up-5min` · User `root` · ติ๊ก Enabled

แท็บ **Schedule** — ต้องตั้งครบทั้งห้าช่อง ไม่ใช่แค่ช่องเดียว:

| ช่อง | ค่าที่ต้องตั้ง | ถ้าตั้งผิดจะเป็นอย่างไร |
|---|---|---|
| Run on the following days | `Daily` | — |
| **Start time** | **`00:00`** | ตั้งเป็น `23:59` → รันวันละครั้งเดียว = ไม่มีตาข่ายเลย |
| Continue running within the same day | ✓ **ติ๊ก** | ไม่ติ๊ก → รันวันละครั้งเดียว |
| **Repeat** | **`every 5 minutes`** | DSM เติมให้เองเป็นรายชั่วโมง ต้องเปลี่ยนเอง |
| **Last run time** | **`23:59`** | เป็น `00:59` → เงียบหลังชั่วโมงแรกของทุกวัน |

⚠️ **`Start time` กับ `Last run time` คือขอบสองข้างของหน้าต่างที่งานจะวนซ้ำ
ตั้งให้เท่ากันเมื่อไร หน้าต่างจะยุบจนเหลือรอบเดียว** — งานจะยังขึ้นเขียวในตาราง
ดูเหมือนทำงานปกติทุกอย่าง แต่ไม่ได้เฝ้าอะไรให้เลย เคยพลาดตรงนี้มาแล้วจริง
ทั้งกับงานนี้และกับ DDNS

แท็บ **Task Settings**: ใส่เส้นทางสคริปต์เดียวกับงานที่ 1 · และ **ติ๊ก
`Send run details only when the script terminates abnormally`** พร้อมใส่อีเมล
— `ensure-up.sh` ออกด้วยรหัส 1 เมื่อกู้ไม่สำเร็จ ติ๊กช่องนี้แล้วเคสนั้นจะกลายเป็น
อีเมลเตือน แทนที่จะเป็นความเงียบที่ไม่มีใครรู้จนกว่าแขกจะบ่นว่าเข้าเว็บไม่ได้

เปิดงานนี้ไว้ช่วง **27–30 ส.ค.** แล้วปิดหลังงานจบก็ได้ (หรือปล่อยไว้ก็ไม่เสียหาย
เพราะมันไม่แตะอะไรตอนเว็บปกติ)

---

#### ตรวจว่าตั้งถูกแล้ว โดยไม่ต้องรอให้ถึงเวลา

ดูคอลัมน์ **`Next run time`** ในตารางรายการงาน — อ่านจากหน้าจอเดียวจบ

| แถว | `Next run time` ต้องเป็น |
|---|---|
| `wedding-ensure-up-boot` | **ว่าง / ไม่มีค่า** (ถ้ามีเวลาขึ้น = สร้างผิดชนิด เป็น Scheduled Task) |
| `wedding-ensure-up-5min` | มีเวลา และ **ขยับทุก 5 นาที** เมื่อกดปุ่ม refresh มุมขวาล่าง |

ถ้าแถวที่สองขึ้นเป็นเวลาห่างกันเป็นชั่วโมงหรือขึ้น `23:59` แปลว่าช่อง Start time
หรือ Repeat ยังไม่ถูก — กลับไปดูตารางข้างบน

ซ้อมจริงก่อนงานหนึ่งครั้ง — สั่ง `sudo docker compose down` แล้วรอดูว่าเว็บกลับมาเอง
ภายใน 5 นาที แล้วเปิด `/volume1/wedding/ensure-up.log` ดูว่ามันจดว่ากู้อะไรไปบ้าง
**นี่คือตัวพิสูจน์ว่าตาข่ายใช้ได้ ไม่ใช่การที่ตารางงานหน้าตาถูก**
