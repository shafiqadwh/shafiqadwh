...
boothRouter.post('/api/booth/upload', uploadLimiter, (req, res) => {
   ...
   *
   * **ตรวจก่อน multer ไม่ใช่หลัง** — เดิมตรวจหลัง แปลว่าคำขอที่กุญแจผิดยัง
   * ถูกเขียนลงดิสก์ครบทุกไฟล์ก่อน แล้วค่อยลบทิ้ง · วัดแล้วด้วยคำขอกุญแจผิดหนึ่งครั้ง:
   * ไฟล์ 4 ก้อน 32 MB ลง `data/tmp` จริง ๆ ก่อนได้ 401 · เพดานของ multer คือ
   * 10 ไฟล์ × 25 MB = 250 MB ต่อคำขอ และตัวจำกัดอัตรายอมให้ 600 คำขอ/ชม./ไอพี
   * = ดิสก์ของ NAS ที่บ้านเต็มได้ด้วยคำขอที่ไม่มีกุญแจแม้แต่ดอกเดียว
   */
  if (!keyMatches(req.get('x-booth-key'))) {
    res.status(401).json({ error: 'bad_key' });
    // ปล่อยไบต์ที่เหลือไหลทิ้งไป (ไม่ลงดิสก์) — ตัดซ็อกเก็ตทิ้งกลางคันจะทำให้
    // ฝั่งบูธเห็นเป็น "ต่อไม่ติด" แทนที่จะเป็น "กุญแจไม่ถูกต้อง" ซึ่งคนละทางแก้กัน
    req.resume();
    return;
  }
...
async function receive(req, res, uploadError) {
  ...
  // กุญแจถูกตรวจไปแล้วก่อน multer จะเขียนอะไรลงดิสก์ (ดูคอมเมนต์ที่ปากทางข้างบน)
  if (uploadError) {
  ...
  // สองสายที่โทเคนเดียวกันต้องเดินทีละสาย — เหตุผลอยู่ที่ `oneAtATime`
  return oneAtATime(token, () => store(res, { manifest, token, sheet, shots, gif, everything }));
}

/*
 * คำขอที่โทเคนเดียวกันต้องไม่ทับกัน
 *
 * ไฟล์ของรอบหนึ่งถูกตั้งชื่อจากโทเคน (`<โทเคน>-sheet.jpg`) สองสายที่ส่งโทเคนเดียวกัน
 * พร้อมกันจึงเขียนลงชื่อเดียวกัน · ทั้งคู่ผ่านด่าน "ส่งซ้ำไหม" ไปได้เพราะยังไม่มีแถว
 * แล้วสายที่สอง `insertBoothSession` ล้มด้วย PRIMARY KEY — **แล้วตัวเก็บกวาดของมัน
 * ก็ลบไฟล์ทิ้ง ซึ่งเป็นไฟล์ชุดเดียวกับที่สายแรกเพิ่งบันทึกแถวไว้** ผลคือแถวมีอยู่
 * แต่ไฟล์ไม่มี · แขกสแกน QR แล้วได้หน้าที่โหลดแผ่นไม่ขึ้น และส่งใหม่ก็ไม่ได้
 * เพราะทั้งสองฝั่งคิดว่าเรียบร้อยแล้ว
 *
 * เกิดได้จริงตั้งแต่มีเพดานเวลาฝั่งบูธ: คำขอที่บูธเลิกรอแล้วยังเดินอยู่ฝั่งเซิร์ฟเวอร์
 * ส่วนบูธกดส่งใหม่ — สองสาย โทเคนเดียวกัน · และกดปุ่มส่งสองหน้าต่างก็ได้เหมือนกัน
 *
 * เข้าคิวในโปรเซสพอ เพราะเว็บนี้รันคอนเทนเนอร์เดียวโปรเซสเดียว · สายที่สองจะเห็น
 * แถวของสายแรกตอนถึงคิว แล้วตอบ duplicate ตามปกติ
 */
const queues = new Map();

function oneAtATime(token, work) {
  const earlier = queues.get(token) ?? Promise.resolve();
  const mine = earlier.then(work, work);
  const tail = mine.then(() => {}, () => {});
  queues.set(token, tail);
  // ปล่อยคีย์ทิ้งเมื่อไม่มีใครต่อคิวอยู่ ไม่งั้น Map โตตามจำนวนรอบถ่ายทั้งงาน
  void tail.then(() => {
    if (queues.get(token) === tail) queues.delete(token);
  });
  return mine;
}

async function store(res, { manifest, token, sheet, shots, gif, everything }) {
  /*
