...
# ตรวจแบบไม่พึ่งช่วงอักขระใน regex
#
# เดิมใช้ `grep '^[ -~]\{16,\}$'` ซึ่ง `[ -~]` เป็น **ช่วง** ที่ grep ตีความตาม
# ลำดับการเรียงของ locale ไม่ใช่ตามรหัสอักขระ — ผลจึงต่างกันได้ตามเครื่องและ
# ภาษาที่ตั้งไว้ · เกิดขึ้นจริงบน Ubuntu 26.04: กุญแจ 32 ตัวที่เป็นตัวอักษรอังกฤษ
# กับตัวเลขล้วน ถูกตัดสินว่าใช้ไม่ได้ ทั้งที่ถูกต้องทุกประการ
#
# วิธีใหม่ไม่มีช่วงให้ตีความ: นับความยาวด้วยเชลล์ แล้วลบอักขระที่พิมพ์ได้ทิ้ง
# ให้หมด เหลืออะไรแปลว่ามีตัวที่ส่งเป็น HTTP header ไม่ได้ปนอยู่
key_ok() {
  [ "${#1}" -ge 16 ] || return 1
  [ -z "$(printf '%s' "$1" | LC_ALL=C tr -d '[:print:]')" ]
}

if [ -z "$KEY_LINE" ]; then
  warn "BOOTH_KEY ว่าง — บูธจะส่งรูปขึ้นเว็บไม่ได้ · ถูกแล้วถ้าเครื่องนี้รันแต่เว็บ"
elif key_ok "$KEY_LINE"; then
  ok "BOOTH_KEY ใช้ได้ (${#KEY_LINE} ตัว)"
else
  # บอกด้วยว่าเห็นอะไร ไม่ใช่แค่บอกว่าผิด — คนที่เจอบรรทัดนี้อยู่หน้าเครื่องคนเดียว
  bad "BOOTH_KEY ใช้ไม่ได้: ยาว ${#KEY_LINE} ตัว (ต้องอย่างน้อย 16) และต้องเป็น ASCII ที่พิมพ์ได้"
  fails=$((fails + 1))
fi
