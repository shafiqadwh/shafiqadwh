...
**เลือกได้ต่องานว่าจะเปิดภาษาไหนบ้างและภาษาไหนเป็นภาษาหลัก** — งานโรงเรียนใช้ไทย
อย่างเดียว งานฝั่งมาเลย์ขึ้นมลายูก่อน โดยไม่ต้องแยกเครื่องหรือแยกคอนเทนเนอร์
...
| ✅ **ตรวจก่อนส่ง** | เลือกรูปแล้วยังไม่ส่ง — เห็นรูปย่อทุกใบ เอาใบที่กดพลาดออกได้ แล้วค่อยกดยืนยัน |
...
| 📺 **Slideshow ขึ้นจอ** | รูปที่เพิ่งส่งขึ้นจอภายในไม่กี่วินาที แทรกคำอวยพรและ QR เป็นระยะ · **แผ่นที่ photo booth พิมพ์ให้แขกก็ขึ้นจอด้วย** (ปิดได้ในหน้าแอดมิน) |
| 🔗 **เชื่อมต่อทีวีด้วย QR** | ทีวีขึ้นรหัส 6 ตัว เจ้าภาพสแกนด้วยกล้องมือถือแล้วเลือกโหมด ทีวีเข้าสไลด์โชว์เอง — **ไม่ต้องพิมพ์ URL ด้วยรีโมต** และรีบูตแล้วไม่ต้องจับคู่ใหม่ |
...
| 🎵 **เพลงคลอในหน้าเว็บ** | เลือกจากคลังเพลงชุดเดียวกับที่หนังใช้ · แขกกดเองถึงจะเล่น ไม่ดึงไฟล์เสียงโดยไม่ได้ขอ (ปุ่มถอดออกจากหน้าแขกไว้ก่อน — ตัวเลือกในแอดมินยังใช้ได้) |
| 🛡 **จัดการหน้างาน** | เลือกลบหลายรูปพร้อมกันจากมือถือ ลบแล้วกด **เลิกทำ** ได้ทันที (เข้าถังขยะก่อน 7 วัน) เปิด-ปิดการอัพโหลด และโหมดตรวจก่อนโชว์ |
...
ครอบคลุมตั้งแต่คีย์ภาษาครบทุกไฟล์และไม่มีคำแปลตกหล่น, ตรวจชนิดไฟล์จาก magic bytes
(รวมไฟล์ปลอมนามสกุล), อัพโหลดรูปและวิดีโอจริงตั้งแต่ต้นจนจบ, การแปลง HEVC → H.264,
สิทธิ์เข้าหน้า admin, การสร้าง ZIP, rate limit, การแยกงานออกจากกันในระบบหลายงาน
ไปจนถึงเทสต์ที่ขับเบราว์เซอร์และ Electron จริง

**คำสั่งเดียวรันสองรอบโดยตั้งใจ** — เทสต์ของเว็บรันขนานตามปกติ ส่วนเทสต์ของ
photo booth ถูกจำกัดไว้ที่สองไฟล์พร้อมกัน (`--test-concurrency=2`) เพราะมีเจ็ดไฟล์
ที่เปิด Electron จริง และเครื่องที่รันมี 4 คอร์ · ปล่อยให้ชนกันสี่ตัวพร้อมกันแล้ว
**เทสต์ล้มแบบสุ่มประมาณหนึ่งในสามรอบ** ซึ่งอันตรายกว่าเทสต์ที่ช้า: พอความล้ม
กลายเป็นเรื่องปกติ ความล้มของจริงก็ถูกมองข้ามไปด้วย · วัดแล้ว 77 วินาที (ชนกัน)
เทียบกับ 102 วินาที (แยกรอบ) — จ่ายเพิ่ม 25 วินาทีเพื่อให้ผลลัพธ์เชื่อได้
