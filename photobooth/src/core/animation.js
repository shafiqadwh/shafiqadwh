...
/*
 * 128 สีแทนเพดาน 256 ของ GIF
 *
 * วัดกับภาพขนาดที่เว็บแคมส่งมาจริง: 1062 KB → 478 KB และเข้ารหัสเร็วขึ้นสามเท่า
 * (624 → 190 มิลลิวินาที) โดย **มองด้วยตาแล้วไม่เห็นความต่าง** เพราะ dithering
 * เกลี่ยให้แล้ว — เทียบภาพไล่เฉดซึ่งเป็นกรณีที่แย่ที่สุดของการลดจานสี
 *
 * ลองบีบข้ามเฟรม (`interFrameMaxError`) ด้วยแล้ว ได้เล็กกว่านี้อีกก็จริง แต่
 * **ทิ้งเงาจาง ๆ ของเฟรมก่อนไว้บนพื้นหลัง** ซึ่งเห็นชัดบนฉากเรียบ ๆ · ของที่แขก
 * เก็บไว้ไม่ควรมีตำหนิที่มองเห็นได้ เพื่อแลกกับไบต์ที่ไม่มีใครบ่น
 */
...
export async function makeGif(photos, {
  ...
  return sharp(order, { join: { across: 1, animated: true } })
    ...
    .gif({ loop: 0, colours: GIF_COLOURS, delay: order.map(() => delayMs) })
    .toBuffer();
}
