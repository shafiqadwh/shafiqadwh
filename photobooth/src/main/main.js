...
/*
 * กลับไปหน้าพร้อมถ่ายเองเมื่อไม่มีใครแตะ
 *
 * แขกเดินจากไปกลางคันเป็นเรื่องปกติที่สุดของบูธ — ได้กระดาษแล้วเดินไปหาโต๊ะตัวเอง
 * โดยไม่กดอะไรอีก · ของเดิมจอค้างอยู่ที่หน้านั้นตลอดไป ซึ่งเสียสองอย่างพร้อมกัน
 *
 *   1. **QR ของคนก่อนค้างอยู่บนจอให้คนถัดไปสแกน** = รูปของแขกคนหนึ่งไปอยู่ในมือ
 *      อีกคน · เป็นความผิดพลาดชนิดเดียวกับที่กันไว้แล้วทุกที่ในฝั่งเว็บ
 *   2. คนที่เดินมาเห็นจอที่มีรูปคนอื่นค้างอยู่ไม่รู้ว่าบูธว่าง แล้วเดินผ่านไป
 *
 * ไม่ตั้งเวลาให้ขั้น `pay` โดยตั้งใจ — แขกกำลังเปิดแอปธนาคารอยู่ การรีเซ็ตตรงนั้น
 * คือการยกเลิกการขายกลางคัน · และขั้น `shoot` ก็ไม่ตั้ง เพราะมีคนยืนถ่ายอยู่
 *
 * `BOOTH_IDLE_MS` มีไว้ให้เทสต์ย่นเวลารอ ไม่ใช่ให้ตั้งใช้งานจริง
 */
const IDLE_OVERRIDE = Number(process.env.BOOTH_IDLE_MS) || 0;
const IDLE_MS = Object.freeze({
  // ได้ของแล้ว · ยาวพอให้สแกน QR ทัน (ราว 15 วินาที) แต่ไม่ปล่อยให้ค้างข้ามคน
  done: IDLE_OVERRIDE || 120000,
  // กำลังตัดสินใจว่าจะเอาไหม — ให้เวลามากกว่า
  review: IDLE_OVERRIDE || 150000,
  /*
   * ยืนจัดท่าอยู่หน้าภาพสด · **ยาวที่สุดในสามค่านี้โดยตั้งใจ**
   *
   * ขั้นนี้ไม่มีอะไรเร่งแขกเลย กลุ่มใหญ่จัดแถว ถอดแว่น เรียกคนที่ยังเดินมาไม่ถึง
   * ใช้เวลาได้เป็นนาที · ตัดกลางคันแปลว่าต้องเริ่มจัดใหม่ทั้งกลุ่ม ซึ่งแย่กว่า
   * ปล่อยให้เว็บแคมติดอยู่อีกหนึ่งนาทีอย่างเทียบกันไม่ได้
   */
  frame: IDLE_OVERRIDE || 180000,
});
...
ipcMain.handle('booth:setup', async () => {
  ...
  return {
     ...
     */
    // เวลาที่จอจะกลับไปหน้าพร้อมถ่ายเองเมื่อไม่มีใครแตะ (ดู IDLE_MS)
    idle: IDLE_MS,
