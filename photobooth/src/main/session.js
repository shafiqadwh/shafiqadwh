...
export async function saveSession(root, { token, photos, sheet, gif, settings, effect, template }) {
  ...
  try {
    ...
    const manifest = {
      ...
      /*
       * รอบนี้ถูกพิมพ์ไว้ให้ส่งขึ้น **เว็บไหน**
       *
       * บูธตัวเดียววิ่งหลายงาน และรอบถ่ายถูกส่งขึ้นเว็บทีหลัง (มักเป็นวันรุ่งขึ้น)
       * ถ้าเจ้าของรับงานที่สองแล้วเปลี่ยนที่อยู่เว็บในหน้าตั้งค่าก่อนจะได้กดส่งงานแรก
       * **รอบของงานแรกจะไหลขึ้นเว็บของงานที่สอง** — รูปของลูกค้าคนหนึ่งไปโผล่ใน
       * อัลบั้มของลูกค้าอีกคน ซึ่งเอากลับไม่ได้เมื่อเกิดแล้ว
       *
       * จดปลายทางไว้ตั้งแต่ตอนพิมพ์ (ซึ่งเป็นตอนที่ QR บนกระดาษถูกตัดสินไปแล้ว)
       * **ไม่เก็บกุญแจลงไฟล์นี้** — ที่อยู่พอสำหรับบอกว่าใช่งานเดียวกันหรือเปล่า
       * ค่าว่าง = ตอนพิมพ์ยังไม่ได้ตั้งที่อยู่ จึงไม่มีปลายทางที่ตั้งใจไว้ให้เทียบ
       */
      uploadTo: settings.baseUrl || null,
      template,
      effect,
