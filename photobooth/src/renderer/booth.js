...
async function shoot() {
  ...
  /*
   * **ไม่ปิดกล้องเมื่อจบรอบ** — สลับจากของเดิมที่ปิดทุกทางออก
   *
   * ของเดิมปิดเพื่อไม่ให้ไฟกล้องติดค้างหลังจอกลับไปหน้าพร้อม ซึ่งเป็นเหตุผลที่ดี
   * ตอนพรีวิวมาจากเว็บแคม · แต่กับชุด HDMI + capture card การปล่อยอุปกรณ์คือการ
   * ตัดสัญญาณเข้า แล้วรอบถัดไปต้องจับการ์ดใหม่ ซึ่งเป็นจังหวะที่ล้มบ่อยที่สุดของ
   * ชุดนั้น — บูธที่ถ่ายรอบสองไม่ได้ แย่กว่าไฟเว็บแคมที่ติดค้างอย่างเทียบกันไม่ได้
   *
   * ราคาที่ยอมจ่าย: ไฟกล้องติดตลอดเวลาที่บูธเปิดอยู่ · กล้องถูกคืนให้ระบบตอนปิด
   * หน้าบูธ (`beforeunload` ท้ายไฟล์) และตอนสัญญาณขาดแล้วต้องต่อใหม่
   */
  startRelay();
  try {
...
/**
 * จัดท่า — ภาพสดเต็มจอ พร้อมปุ่มและนาฬิกาที่เป็น **เพดาน ไม่ใช่การบังคับรอ**
 *
 ...
 *
 * ขั้นนี้จึงให้เวลาจัดแถวเต็มที่ แขกเห็นตัวเองเต็มจอ ขยับจนพอใจ แล้วกดเองเมื่อพร้อม
 * ส่วนการนับถอยหลังสามวินาทียังอยู่ที่เดิม ทำหน้าที่เดิมคือบอกจังหวะกดชัตเตอร์
 *
...
/**
 * นาฬิกาบนขั้นจัดท่า — นับลงแล้ว **ถ่ายเอง** เมื่อหมดเวลา
 *
 * บูธจริงเป็นจอสัมผัสล้วน ไม่มีเมาส์ไม่มีคีย์บอร์ด และแขกที่ยืนอยู่หน้ากล้องมัก
 * ไม่กล้าแตะจอเองเพราะกลัวกดผิด · ถ้าไม่มีอะไรเดินหน้าให้ บูธจะค้างอยู่ตรงนั้น
 * จนกว่าจะมีคนเดินมาช่วย ซึ่งเท่ากับไม่มีบูธ
 *
 * เห็นตัวเลขนับลงด้วย ไม่ใช่แค่เดินเงียบ ๆ — คนจะได้รู้ว่าเหลือเวลาเท่าไร
 * และ **กดเองได้ตลอดเวลาเพื่อไม่ต้องรอจนครบ** นาฬิกาเป็นเพดาน ไม่ใช่การบังคับรอ
 */
let frameTimer = null;
...
function startFrameClock() {
  ...
  frameTimer = setInterval(() => {
    // ขั้นเปลี่ยนไปแล้ว (มีคนกด หรือรอบถูกทิ้ง) = นาฬิกานี้ไม่ใช่ของใครแล้ว
    if (body.dataset.stage !== 'frame') {
      stopFrameClock();
      return;
    }
    left -= 1;
    el('frame-left').textContent = String(Math.max(0, left));
    if (left <= 0) {
      stopFrameClock();
      if (!state.busy) shoot();
    }
...
const ACTIONS = {
  ...
  back: {
    ...
    frame: () => { stopFrameClock(); return reset({ keep: holdingPaid() }); },
